...
function zoomMap(factor, centerX, centerY) {
  ...
  mapTransform.x = pointerX - worldX * nextScale;
  mapTransform.y = pointerY - worldY * nextScale;
...
if (mapViewport) {
  ...
  mapViewport.addEventListener('pointermove', (event) => {
    ...
    if (activePointers.size >= 2 && pinchState) {
      ...
      mapTransform.scale = nextScale;
      mapTransform.x = center.x - pinchState.worldX * nextScale;
      mapTransform.y = center.y - pinchState.worldY * nextScale;
      scheduleMapTransform();
      return;
    }

    if (activePointers.size === 1 && panState && panState.pointerId === event.pointerId) {
      mapTransform.x = panState.startX + (point.x - panState.startPointer.x);
      mapTransform.y = panState.startY + (point.y - panState.startPointer.y);
      scheduleMapTransform();
    }
  });
